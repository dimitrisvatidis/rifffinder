  ...
  useEffect(() => {
    const fetchRequests = async () => {
      try {
        ...
        const musicianIds = [
          ...new Set(requestsResponse.data.map((req) => req.musicianId)),
        ];
        const postingIds = [
          ...new Set(requestsResponse.data.map((req) => req.postingId)),
        ];
        const bandIds = [
          ...new Set(requestsResponse.data.map((req) => req.bandId)),
        ];
        ...
        const postingDetails = await Promise.all(
          postingIds.map(async (id) => {
            const res = await axios.get(
              `https://localhost:7290/api/postings/${id}`,
              { headers }
            );
            return { id, ...res.data };
          })
        ...
        const bandDetails = await Promise.all(
          bandIds.map(async (id) => {
            const res = await axios.get(
              `https://localhost:7290/api/bands/${id}`,
              { headers }
            );
            return { id, ...res.data };
          })
    ...
    <div className="d-flex flex-column min-vh-100">
      <div className="container mt-5 flex-grow-1">
        ...
        {pendingIncomingRequests.map((req) => (
          <div key={req.id} className="card shadow-lg mb-4 border-0">
            <div className="card-header bg-dark text-white fs-5 fw-semibold">
              {musiciansData[req.musicianId]?.name || 'Unknown'} wants to join
              your band
            </div>
            <div className="card-body fs-5">
              ...
              <p>
                ...
                {postingsData[req.postingId]?.title || 'Unknown'}
              </p>
        ...
        {pendingOutgoingRequests.map((req) => (
          <div key={req.id} className="card shadow-lg mb-4 border-0">
            <div className="card-header bg-secondary text-white fs-5 fw-semibold">
              ...
              {bandsData[req.bandId]?.name || 'Unknown'}
            </div>
            <div className="card-body fs-5">
              <p>
              ...
              <p>
                ...
                <span
                  className={`badge ${getStatusBadgeClass(req.status)} fs-6`}
                >
        ...
        {completedRequests.map((req) => (
          <div key={req.id} className="card shadow-lg mb-4 border-0">
            <div
              className={`card-header ${getStatusBadgeClass(req.status)} text-white fs-5 fw-semibold`}
            >
              {musiciansData[req.musicianId]?.name || 'Unknown'}, (
              {musiciansData[req.musicianId]?.instrument || 'Unknown'}), applied
              for {postingsData[req.postingId]?.title || 'Unknown'}, for the
              band {bandsData[req.bandId]?.name || 'Unknown'}
            </div>
            <div className="card-body fs-5">
              <p>
                <strong>Status:</strong> {getStatusText(req.status)}
              </p>
            </div>
          </div>
        ))}
