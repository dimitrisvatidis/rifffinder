  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const postingsResponse = await axios.get("https://localhost:7290/api/postings", { headers });
        setPostings(postingsResponse.data);

        const musicianResponse = await axios.get("https://localhost:7290/api/musicians/me", { headers });
  ...
  <div className="d-flex flex-column min-vh-100 bg-light">
      <div className="container mt-4 flex-grow-1">
        <div className="text-center mb-4">
          ...
          <p className="text-muted">Find and explore the latest opportunities in the music world.</p>
        </div>

        {belongsToBand && (
          <div className="text-center mb-4">
            <button onClick={handleAddPosting} className="btn btn-primary btn-lg">
              ➕ Add Posting
            </button>
          </div>
        )}
              ...
              <div key={posting.id} className="col">
                <div
                  className={`card shadow-sm h-100 border-0 rounded-4 ${
                    musician?.bandId === posting.bandId ? "border-primary border-2" : ""
                  }`}
                >
                  <div className="card-body d-flex flex-column">
                    <h5 className="card-title text-primary fw-bold">{posting.title}</h5>
        ...
        {postings.filter((posting) => posting.status === 0).length === 0 && (
          <div className="text-center mt-5">
            <h5 className="text-muted">No open postings are available at the moment.</h5>
          </div>
        )}
      </div>
